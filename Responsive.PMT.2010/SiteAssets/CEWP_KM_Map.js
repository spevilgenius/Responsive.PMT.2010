...
function getMapPopupData() {
	...
	for (i = 0; i < mapitems.length; i++) {
		...
		var fields = ['OrganizationType', 'Level' , 'Base', 'POC', 'Title'];
		deferreds.push($.when(CKO.CSOM.GetListItems.getitemsfilteredorderedandpassfieldstoelement ("current", "Organization", "Base", base , "ChartOrder", i, fields)).then(function (items, i) {
	    	if (items.get_count() > 0) {
	    		...
	    		while (enumerator.moveNext()){
	    			...
	    			if(cnt == 0) {
	    				mapitems[i].PopupData =  current.get_item("OrganizationType") + ";" + current.get_item("Level") + ";" + current.get_item("Base") + ";" + current.get_item("Title") + ";" + current.get_item("POC");
	    			}
	    			else {
	    				mapitems[i].PopupData += "|" + current.get_item("OrganizationType") + ";" + current.get_item("Level") + ";" + current.get_item("Base") + ";" + current.get_item("Title") + ";" + current.get_item("POC");
...
function getMapPopupData2() {
	...
	for (i = 0; i < mapitems2.length; i++) {
		...
		var fields = ['OrganizationType', 'Level' , 'Base', 'POC', 'Title'];
		deferreds2.push($.when(CKO.CSOM.GetListItems.getitemsfilteredorderedandpassfieldstoelement ("current", "Organization", "Base", base , "ChartOrder", i, fields)).then(function (items, i) {
	    	if (items.get_count() > 0) {
	    		var enumerator = items.getEnumerator();
				var cnt = 0;
	    		while (enumerator.moveNext()){
	    			var current = enumerator.get_current();
	    			if(cnt == 0) {
	    				mapitems2[i].PopupData =  current.get_item("OrganizationType") + ";" + current.get_item("Level") + ";" + current.get_item("Base") + ";" + current.get_item("Title") + ";" + current.get_item("POC");
	    			}
	    			else {
	    				mapitems2[i].PopupData += "|" + current.get_item("OrganizationType") + ";" + current.get_item("Level") + ";" + current.get_item("Base") + ";" + current.get_item("Title") + ";" + current.get_item("POC");
...
function getMapPopupData3() {
	...
	for (i = 0; i < mapitems3.length; i++) {
		...
		var fields = ['OrganizationType', 'Level' , 'Base', 'POC', 'Title'];		
		deferreds3.push($.when(CKO.CSOM.GetListItems.getitemsfilteredorderedandpassfieldstoelement ("current", "Organization", "Base", base , "ChartOrder", i, fields)).then(function (items, i) {
	    	if (items.get_count() > 0) {
	    		var enumerator = items.getEnumerator();
				var cnt = 0;
	    		while (enumerator.moveNext()){
	    			var current = enumerator.get_current();
	    			if(cnt == 0) {
	    				mapitems3[i].PopupData =  current.get_item("OrganizationType") + ";" + current.get_item("Level") + ";" + current.get_item("Base") + ";" + current.get_item("Title") + ";" + current.get_item("POC");
	    			}
	    			else {
	    				mapitems3[i].PopupData += "|" + current.get_item("OrganizationType") + ";" + current.get_item("Level") + ";" + current.get_item("Base") + ";" + current.get_item("Title") + ";" + current.get_item("POC");
...
function getMapPopupDatahq() {
	...
	for (i = 0; i < mapitemshq.length; i++) {
		...
		var fields = ['OrganizationType', 'Level' , 'Base', 'POC', 'Title'];
		deferredshq.push($.when(CKO.CSOM.GetListItems.getitemsfilteredorderedandpassfieldstoelement ("current", "Organization", "Base", base , "ChartOrder", i, fields)).then(function (items, i) {
	    	if (items.get_count() > 0) {
	    		var enumerator = items.getEnumerator();
				var cnt = 0;
	    		while (enumerator.moveNext()){
	    			var current = enumerator.get_current();
	    			if(cnt == 0) {
	    				mapitemshq[i].PopupData =  current.get_item("OrganizationType") + ";" + current.get_item("Level") + ";" + current.get_item("Base") + ";" + current.get_item("Title") + ";" + current.get_item("POC");
	    			}
	    			else {
	    				mapitemshq[i].PopupData += "|" + current.get_item("OrganizationType") + ";" + current.get_item("Level") + ";" + current.get_item("Base") + ";" + current.get_item("Title") + ";" + current.get_item("POC");
